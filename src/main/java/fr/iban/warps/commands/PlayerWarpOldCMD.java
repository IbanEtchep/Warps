...
public class PlayerWarpOldCMD implements CommandExecutor, TabCompleter {
    ...
    public PlayerWarpOldCMD(WarpsPlugin plugin) {
        ...
        if (sender instanceof Player) {
            ...
            if (args.length == 1) {
                ...
                if (args[0].equalsIgnoreCase("visit")) {
                    List<String> warplist = manager.getPlayerWarps().values().stream().filter(warp -> warp.isOpened() && getName(warp.getOwner()) != null).map(warp ->getName(warp.getOwner())).collect(Collectors.toList());
    ...
    private String getName(UUID uuid) {
		namesCache.putIfAbsent(uuid, Bukkit.getOfflinePlayer(uuid).getName());
